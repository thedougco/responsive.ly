...
In Anthony's post, he notes the [iPhone landing page](http://www.apple.com/iphone/) uses a vertical media query to adjust the height of the hero shot. He also asks readers for examples via Twitter submission. I thought this topic could use a little a more exploration, so I collected and categorized some different use cases for vertical media queries. These examples are pulled both from the Twitter responses to Anthony's post and other posts and demos on different sites.
